...
func (a *AMQP) Start(log *logrus.Entry) error {
	err := backoff.Retry(a.connect, backoff.NewExponentialBackOff())
	if err != nil {
		return err
	}

	go a.notifyWhenClosed(log)
	return nil
}
...
func (a *AMQP) notifyWhenClosed(log *logrus.Entry) {
	...
	reconnection := func() error {
		...
		if err != nil {
			log.Errorln("Error on Dial func, Cannot connect to KNoT: ", err, "Will retry after", reconnectionBackOff.NextBackOff()/time.Second, "seconds")
			return err
		}
		...
		if err != nil {
			log.Errorln("Error to get a channel, Cannot connect to KNoT: ", err, "Will retry after", reconnectionBackOff.NextBackOff()/time.Second, "seconds")
			return err
		}
		log.Println("Reconnection to KNoT amqp was successful")
		a.channel = channel
